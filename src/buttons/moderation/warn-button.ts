...
export class WarnUserButton implements Button {
    ...
    public async execute(intr: ButtonInteraction, data: EventData): Promise<void> {
        ...
        const dropdownMessage = await intr.reply({
            content: 'Please select a reason for the warning:',
        ...
        const filter = (i: StringSelectMenuInteraction) =>
            i.customId === 'warn_reason' && i.user.id === intr.user.id;
        ...
        try {
            ...
            if (selectMenuInteraction && selectMenuInteraction.isStringSelectMenu()) {
                ...
                await ModerationUtils.warnUser(
                    intr,
                    targetMember,
                ...
                await selectMenuInteraction.update({
                    content: `<@${targetMember.user.id}> has been warned for: ${selectedReason}.`,
